...
function Extract-PrefetchStrings {
    ...
    if ($results.Count -eq 0) { $sb = New-Object System.Text.StringBuilder; foreach ($b in $bytes) { if ($b -ge 32 -and $b -le 126) { [void]$sb.Append([char]$b) } else { [void]$sb.Append(' ') } }; $asciiParts = $sb.ToString() -split '\s+' | Where-Object { $_.Length -gt 2 }; $results += $asciiParts }
...
function Find-InZip {
    ...
    $found = @()
    try { Add-Type -AssemblyName System.IO.Compression.FileSystem -ErrorAction SilentlyContinue; $zip = [System.IO.Compression.ZipFile]::OpenRead($ZipPath); foreach ($entry in $zip.Entries) { if ($entry.FullName -match $Regex) { $found += $entry.FullName } }; $zip.Dispose() } catch {}
    return $found
}
...
if (-not $javaProcs -or $javaProcs.Count -eq 0) { I "No java/javaw processes found." } else { foreach ($p in $javaProcs) { $pid = $p.ProcessId; $cmd = $p.CommandLine; $exe = $p.ExecutablePath; $owner = try { ($p | Invoke-CimMethod -MethodName GetOwner).User } catch { "Unknown" }; $susp = if ($cmd -and ($cmd -match $targetRegex)) { "[SUSPICIOUS]" } else { "[OK]" }; if ($susp -eq "[SUSPICIOUS]") { Y "$susp PID $pid $($p.Name) Owner:$owner" } else { G "$susp PID $pid $($p.Name) Owner:$owner" }; I "    Path: $exe"; I "    Cmd : $cmd`n" } }
...
foreach ($root in $pathsToScan) {
    ...
    try { $files = Get-ChildItem -Path $root -Recurse -Force -ErrorAction SilentlyContinue | Where-Object { -not $_.PSIsContainer -and ($_.Extension -in '.jar','.zip','.exe') } } catch { I "  Unable to scan $root: $($_.Exception.Message)"; continue }
    $total = $files.Count
    ...
    foreach ($file in $files) {
        ...
        $name = $file.Name; $full = $file.FullName; $flagged = $false; $matches = @()
        if ($name -match $targetRegex) { $matches += "NameMatch"; $flagged = $true }
        if ($file.Extension -match '\.jar|\.zip') { $inner = Find-InZip -ZipPath $full -Regex $targetRegex; if ($inner.Count -gt 0) { $matches += ($inner | ForEach-Object { "Inner:$_" }); $flagged = $true } }
        if ($flagged) { Y ("[FLAG] {0}  ({1})" -f $name, $full); foreach ($m in $matches) { I ("       {0}" -f $m) } } else { G ("{0} (ok)" -f $name) }
    }
...
if (Test-Path $prefetchFolder) {
    ...
    if ($totalPf -eq 0) { I "  No java/minecraft prefetch files found." } else {
        ...
        foreach ($pf in $pfList) {
            ...
            if ($hits.Count -gt 0) { foreach ($h in $hits | Select-Object -Unique) { Y ("  Prefetch hit: {0}" -f $h) } } else { G "  No suspicious strings found in $($pf.Name)" }
        }
        Write-Progress -Activity "Parsing prefetch files" -Completed
    }
} else { I "Prefetch folder not found: $prefetchFolder" }
