# ==========================
# Minecraft Ghost Client Scanner (All-in-One)
# ==========================
...
if (-not $isAdmin) { R "`n[!] Run as Administrator!"; exit 1 }
...
foreach ($root in $pathsToScan) {
    if (Test-Path $root) {
        I "Scanning: $root ..."
        $allFiles = Get-ChildItem -Path $root -Recurse -Include $extensions -ErrorAction SilentlyContinue
        $total = $allFiles.Count
        $count = 0

        foreach ($file in $allFiles) {
            $count++
            Write-Host ("  [{0}/{1}] Scanning {2}" -f $count,$total,$file.Name) -ForegroundColor Gray

            $matches = @()
            if ($file.Name -match $targetRegex) { $matches += "NameMatch" }
            ...
            if ($matches.Count -gt 0) {
                Y "[FLAG] $($file.Name) Path:$($file.FullName)"
...
if (Test-Path $pfFolder) {
    $pfFiles = Get-ChildItem -Path $pfFolder -Filter "*.pf" | Where-Object { $_.Name -match "MINECRAFT|JAVA|JAVAW" }
    $totalPf = $pfFiles.Count
    $pfCount = 0

    foreach ($pf in $pfFiles) {
        $pfCount++
        I ("[{0}/{1}] Parsing: {2}" -f $pfCount,$totalPf,$pf.Name)
